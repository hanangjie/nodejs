	...
	try{
		request.setEncoding('utf-8');
		 request.addListener("data",function(postdata){
			...
			var selectSQL = 'select * from t_user where name=\''+params['name']+'\'';
			var insertSQL="insert into t_user(name,pwd) values('"+params['name']+"','"+params['pwd']+"')";
			sqlIn.sqlInImg(selectSQL,call_return_sql,response,request,insertSQL);//数据库操作
...
function call_return_sql(res,response,request,insertSQL){
	if(res.length==0){
		sqlIn.sqlIn(insertSQL,call_return_insql,response);//数据库操作
		response.writeHead(200, {"Content-Type": "text/html"});
...
function index(response,request){
		...
		var ls_html='<html xmlns="http://www.w3.org/1999/xhtml">'+
					...
					'<input type="file" name="upload" multiple="multiple" rel="up_img">'+
